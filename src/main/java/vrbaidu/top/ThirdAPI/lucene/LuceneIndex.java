package vrbaidu.top.ThirdAPI.lucene;
...
import vrbaidu.top.util.StringUtil;
...
public class LuceneIndex {
	...
	public void addIndex(User user)throws Exception{
		 ...
		 */
		doc.add(new TextField("username", user.getNickname(), Field.Store.YES));
		doc.add(new TextField("description",user.getEmail(), Field.Store.YES));
	...
	public List<User> searchBlog(String q)throws Exception{
		...
		for(ScoreDoc scoreDoc:hits.scoreDocs){
			...
			user.setEmail(doc.get(("description")));
			...
			if(description!=null){
				...
				if(StringUtil.isEmpty(hContent)){
					if(description.length()<=200){
						...
						user.setEmail(description.substring(0, 200));
					}
				}else{
					user.setEmail(hContent);
				}
			}
			userList.add(user);
		}
